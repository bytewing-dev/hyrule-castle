...
export function fightSetup(hero: Character, oponent: Character): Character[] {
  console.log(`${hero.name} (${hero.hp}/${hero.hpMax} HP)`);
  console.log(`${oponent.name} (${oponent.hp}/${oponent.hpMax} HP)\n`);
  hero = {
    ...hero,
    protect: false,
  }
  hero.damage = hero.str;
  oponent.damage = oponent.str;
  return [hero, oponent];
}
